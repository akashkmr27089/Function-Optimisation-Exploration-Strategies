...
class Selection:
    def __init__(self, nos_selection, population, number_of_selected_population):
    ...
    def population_to_probability(self, population):
        ...
        selection_prob = [self.pop_control.calculate_fitness(x)/sum_val for x in population]
...
class GeneticAlgorithm:
    def __init__(self, size_of_each_population, population_batch_size,
        number_top_population, max_possible_value, selection_population, population_generation, calculate_fitness):

        replacement = number_top_population
        self.calculate_fitness = calculate_fitness
        self.population_cal = Population(self.calculate_fitness)
        self.selection = Selection(replacement, self.population_cal, selection_population)
    ...
    def train(self, iteration, type_selection='roulette'):
        for _ in range(iteration):
            sorted_pop = sorted(self.population, key=self.calculate_fitness, reverse=True)[:self.population_batch_size]
